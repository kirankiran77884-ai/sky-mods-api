...
// Ensure the admin account exists and matches the current Render environment variables.
// This runs at every startup so an existing persistent SQLite database is also updated.
function ensureAdminUser() {
  const username = String(process.env.ADMIN_USERNAME || "admin").trim();
  const password = String(process.env.ADMIN_PASSWORD || "ChangeThisAdminPassword");
  if (!username || !password) throw new Error("ADMIN_USERNAME and ADMIN_PASSWORD must not be empty");

  const passwordHash = bcrypt.hashSync(password, 12);
  const existing = db.prepare("SELECT id FROM users WHERE username=?").get(username);
  if (existing) {
    db.prepare("UPDATE users SET password_hash=?, role='admin', enabled=1 WHERE id=?")
      .run(passwordHash, existing.id);
    // Password/config changes invalidate old admin sessions.
    db.prepare("UPDATE sessions SET revoked=1 WHERE user_id=?").run(existing.id);
    console.log(`Admin user synchronized: ${username}`);
  } else {
    db.prepare("INSERT INTO users(username,password_hash,role,enabled) VALUES(?,?,?,1)")
