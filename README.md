...
The server automatically creates or synchronizes the admin account from `ADMIN_USERNAME` and `ADMIN_PASSWORD` at startup. This also works with an existing persistent SQLite database.
...
On every server startup, the account is created if missing or its password/role/enabled state is synchronized if it already exists. Keep these values private and use a strong password in production.
